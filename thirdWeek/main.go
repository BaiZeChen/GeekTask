...
import (
	"GeekTask/thirdWeek/annotation"
	"GeekTask/thirdWeek/http"
	"fmt"
	"go/ast"
	"go/parser"
	"go/token"
...
func parseFiles(srcFiles []string) ([]http.ServiceDefinition, error) {
	...
	for _, src := range srcFiles {
		...
		parseFile, err := parser.ParseFile(fset, src, nil, parser.ParseComments)
		if err != nil {
			return nil, err
		}
		demo := &annotation.SingleFileEntryVisitor{}
		ast.Walk(demo, parseFile)
		file := demo.Get()
